...
class DataSim(Module, AutoCSR):
    ...
    """
    def __init__(self, pads, cmds_sim, *, cd_dq_wr, cd_dq_rd, cd_dqs_wr, cd_dqs_rd, cl, cwl, clk_freq, log_level, bl_max, prefix):
        self.submodules.log = log = SimLogger(log_level=log_level, clk_freq=clk_freq)
        self.log.add_csrs()

        dq_dqs_ratio = len(getattr(pads, prefix+'dq')) // len(getattr(pads, prefix+'dqs_t'))
        if dq_dqs_ratio == 8:
