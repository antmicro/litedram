...
class S7LPDDR5PHY(LPDDR5PHY, S7Common):
    def __init__(self, pads, *, iodelay_clk_freq, with_odelay, **kwargs):
        ...
        half_sys8x_taps = math.floor(self.twck / (4 * iodelay_tap_average))
        ...
        self.oserdese2_sdr(din=~self.out.ck, dout=ck_ser if with_odelay else ck_dly, clk="sys8x")
        if with_odelay:
            self.odelaye2(din=ck_ser, dout=ck_dly, rst=cdly_rst, inc=cdly_inc)
        ...
        for bit in range(len(self.out.ca)):
            ...
            self.oserdese2_sdr(din=self.out.ca[bit], dout=ca_ser if with_odelay else ca_dly, clk="sys8x")
            if with_odelay:
                self.odelaye2(din=ca_ser, dout=ca_dly, rst=cdly_rst, inc=cdly_inc)
        ...
        for byte in range(self.databits//8):
            wck_ser = Signal()
            wck_dly = Signal()
            data_ser(din=self.out.wck[byte], dout=wck_ser if with_odelay else wck_dly, clk="sys8x")
            if with_odelay:
                self.odelaye2(din=wck_ser, dout=wck_dly, rst=cdly_rst, inc=cdly_inc)
